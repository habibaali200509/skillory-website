...
grid.innerHTML = window.CRAFTSMEN.map(c => {
  ...
  const openLbl = c.availability === "Available" ? "Yes" : c.availability === "Busy" ? "Soon" : "No";
  return `
    ...
    <article class="craft-card">
      <div class="top">
        ...
        <div>
          ...
          <div class="meta">
            <span class="stars">${window.starsHTML(c.rating)}</span>
      ...
      <div class="stats">
        ...
        <div class="stat"><span class="v">${openLbl}</span><span class="l">Open</span></div>
      </div>
      <div class="availability"><span class="dot ${dotCls}"></span>${c.availability}</div>
      <div class="actions">
        <a href="profile.html?id=${c.id}" class="btn-orange">View Profile</a>
        <button class="btn-outline-navy">Contact</button>
      </div>
    </article>
  </div>`;
}).join("");
